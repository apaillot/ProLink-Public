...
from Vue.Windows.Calibration.UIProbeSetBaro        import UIProbeSetBaro
...
class VProbe(QObject):
 ...
 siCalibrationPointSelectConnectSignal  = Signal( str, int, dict, QWidget, int, dict )
 ...
 def __init__(self, tParent, tINIFile, tUI):
  ...
  self.tUIProbeSetBaro          = UIProbeSetBaro( tParent )
 ...
 #----------------------------------------------
 def vFVProbeInitMode( self ):
  print("-- vFVProbeInitMode --")
  ...
  # Dashboard - Caché en mode probe
  self.ui.widget_RecordsStored.setVisible(False)
  self.ui.widget_MemoryRemaining.setVisible(False)
  self.ui.widget_BatteryRemaining.setVisible(False)
  self.ui.widget_BatteryVoltage.setVisible(False)
  self.ui.groupBox_EstimatedLogLife.setVisible(False)
  self.ui.groupBox_SondeClock.setVisible(False)
  self.ui.groupBox_SiteIDLocation.setVisible(False)
  self.ui.groupBox_SetupLogRate.setVisible(False)
  self.ui.groupBox_EventLogging.setVisible(False)
  self.ui.widget_DashboardNavBtn.setVisible(False)
  # Liveview - Set probe Baro visible
  self.ui.pushButton_LiveviewSetBaro.setVisible(True)
  # Calib - Set probe Baro visible
  self.ui.setBaroCal_btn.setVisible(True)
  # Conf box
  ...
  # Calibration -
  # Remise à 0 de la valeur des points
 ...
 def vFDisplayGeneralConfiguration( self, ttConfig, sCom ):
  ...
  if( ttConfig["PRODUCT"]["uiModel"] == 30 ):
   ...
   self.vFDisplayGeneralConfigurationAPLite( ttConfig, sCom  )
   return
  #-- Si AquaPlus
  if( ttConfig["PRODUCT"]["uiModel"] == 53 ):
   # Appel de la requête directement
   self.vFDisplayAquaPlusConfiguration( ttConfig, sCom  )
   return
  ...
  # Fenêtre AUX channel select - Capteur AUX sélectionnables
  self.tUIChannelSelect.setProbeSelection()

  #-----------------
  # Liveview
  #-----------------
  #-- Pas de capteur depth --
  # Si AquaPlus / AP-Lite / AP-700 / AP-800 / AP-2000
  if(  ( ttConfig["PRODUCT"]["uiModel"] == 53 )    # AquaPlus
    or ( ttConfig["PRODUCT"]["uiModel"] == 30 )    # AP-Lite
    or ( ttConfig["PRODUCT"]["uiModel"] == 40 )    # AP-700
    or ( ttConfig["PRODUCT"]["uiModel"] == 50 )    # AP-800
    or ( ttConfig["PRODUCT"]["uiModel"] == 10 ) ): # AP-2000
   # Liste des capteurs
   tListSensor = ["TEMP", "Baro", "PH",
  ...
  # CALIBRATION
  #-------------------------------------------
  # Cache le bouton rapidCal
  self.ui.rapidCal_btn.setVisible(True)
  ...
  #
  self.tCalibNavSensorWidget[ "PH" ]  = self.vFCALIBRATIONAddCalibNavElt( "PH",  uiFGetIDWithChannelNameWith("pH"),  ttConfig )
  if( ttConfig["PRODUCT"]["ProBE_SW"] == 3 ):
   self.tCalibNavSensorWidget[ "ORP" ] = self.vFCALIBRATIONAddCalibNavElt( "ORP", uiFGetIDWithChannelNameWith("ORP"), ttConfig )
  self.tCalibNavSensorWidget[ "DO" ]  = self.vFCALIBRATIONAddCalibNavElt( "DO",  uiFGetIDWithChannelNameWith("DO"),  ttConfig )
  self.tCalibNavSensorWidget[ "EC" ]  = self.vFCALIBRATIONAddCalibNavElt( "EC",  uiFGetIDWithChannelNameWith("EC"),  ttConfig )

  # Selon le type de sonde
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 1 ):
   self.tCalibNavSensorWidget[ "AUX1" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX1", 0, ttConfig )
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 2 ):
   self.tCalibNavSensorWidget[ "AUX2" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX2", 1, ttConfig )
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 3 ):
   self.tCalibNavSensorWidget[ "AUX3" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX3", 2, ttConfig )
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 4 ):
   self.tCalibNavSensorWidget[ "AUX4" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX4", 3, ttConfig )
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 5 ):
   self.tCalibNavSensorWidget[ "AUX5" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX5", 4, ttConfig )
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 6 ):
   self.tCalibNavSensorWidget[ "AUX6" ] = self.vFCALIBRATIONAddCalibNavElt( "AUX6", 5, ttConfig )
 ...
 # Affichage configuration
 #----------------------------------------------
 def vFDisplayAquaPlusConfiguration( self, ttConfig, sCom ):
  print("-- vFDisplayAquaPlusConfiguration --")
  ...
  self.ui.label_InterfaceCom.setText( str(sCom) )
  self.ui.label_SWRev.setText( str(ttConfig["PRODUCT"]["sPSW1"]) )
  # Pas d'optical sensor averaging
  self.ui.groupBox_Averaging.setVisible(False)
  # Clear du layout dashboard sensor list
  for i in reversed(range(self.ui.groupBox_DashboardSensors.layout().count())):
   self.ui.groupBox_DashboardSensors.layout().itemAt(i).widget().deleteLater()
  ...
  #-- Pas de capteur depth --
  # Liste des capteurs
  tListSensor = ["TEMP", "Baro", "DO Sat", "DO",   "EC"]
  ...
  # Seulement quand en connexion : non ! sinon pas de refresh des datas en live
  if( self.ui.stackedWidget.currentIndex() == 0 ):
   self.vFCALIBRATIONSensorNavSelectionDynamic("DO",
                                               uiFGetIDWithChannelNameWith("DO"),
                                               ttConfig["SENSOR"]["DO"],
                                               self.tCalibNavSensorWidget["DO"],
                                               ttConfig )
   # Ouverture de l'onglet dashboard
   self.vFNavASBtnClicked(1)
  ##TODO - Prévoir une resélection de l'onglet courant si pas sur connexion
 ...
 # Affichage configuration
 #----------------------------------------------
 def vFDisplayGeneralConfigurationAPLite( self, ttConfig, sCom ):
  print("-- vFDisplayGeneralConfigurationAPLite --")

  #-----------------
  # Suppression inutile
  #-----------------
  # AP-Lite : pas de rapidCal
  self.ui.rapidCal_btn.setVisible(False)
  ...
  if( ttConfig["PRODUCT"]["uiAUXNumber"] >= 1 ):
   self.vFDashboardAddSensorsLine( 0, ttConfig["SENSOR"]["AUX1"] )
  # Fenêtre AUX channel select - Capteur AUX sélectionnables
  self.tUIChannelSelect.setAPLiteSelection()

  #-----------------
  # Liveview
  #-----------------
   # Liste des capteurs
  tListSensor = ["TEMP", "AUX1"]
  self.tListSensor = tListSensor
  # Nettoyage zone vignette valeur physique
  for i in reversed(range(self.ui.widget_11.layout().count())):
   self.ui.widget_11.layout().itemAt(i).widget().deleteLater()
  ...
  # Seulement quand en connexion : non ! sinon pas de refresh des datas en live
  if( self.ui.stackedWidget.currentIndex() == 0 ):
   self.vFCALIBRATIONSensorNavSelectionDynamic("AUX1",
                                               uiFGetIDWithChannelNameWith("AUX1"),
                                               ttConfig["SENSOR"]["AUX1"],
                                               self.tCalibNavSensorWidget["AUX1"],
                                               ttConfig )
   # Ouverture de l'onglet dashboard
   self.vFNavASBtnClicked(1)
  ##TODO - Prévoir une resélection de l'onglet courant si pas sur connexion
 ...
 def vFLiveviewDisplayBloc( self, tChannel, uiColumn, uiRow, uiChannelIndex ):
  ...
  print("---------------------------")
  print('tChannel["sIndex"] = '+ tChannel["sIndex"] )
  print('tChannel["sUnit"]  = '+ tChannel["sUnit"] )
  ...
  print( self.tMeasureBox[len(self.tMeasureBox)-1] )
  print( self.tMeasureBox[len(self.tMeasureBox)-1]["tLabelUnit"].text() )
 ...
 def vFDisplayMeasureData( self, ttConfig ):
  ...
  if( self.uiNavActivePage == 2 ):
   ...
   # Si voie active
   if( ttConfig["SENSOR"]["AUX7"]["sIndex"] != "EMPTY" ):
  ...
  if(   ( self.uiNavActivePage == 4 )
   ...
   for ucCpt in range( len( list(ttMeasure) ) ):
    ...
    print("sIndexTab = "+sIndexTab)
    print(ttMeasure[sIndexTab])
    # Si non défini
    if( sIndexTab == "EMPTY" ): continue
    #%%AP - 2025.03.18 - Ajout
    if( not ttMeasure[sIndexTab]["bActivated"] ): continue
    # Pour la calibration
    if( sIndexTab == "TEMP" ):
 ...
 #----------------------------------------------
 def vFCALIBRATIONAddCalibNavElt( self, sChannelName, ucChannel, ttConfig ):
  print("-- vFCALIBRATIONAddCalibNavElt --")

  tChannel = ttConfig["SENSOR"][sChannelName]
 ...
 def vFCALIBRATIONSensorNavSelectionDynamic( self, sConfigName, ucChannel, tChannel, tWidget, ttConfig ):
  ...
  self.siCalibrationPointSelectConnectSignal.emit( sConfigName, ucChannel, tChannel, self.ui.widget_AuxPt1Value, 0, ttConfig )
  #-- Point 2 --
  if( tCalibChannelInfo[sIndex]["ucPoint"] > 1 ):
   # AP-Lite
   if(   ( int(ttConfig["PRODUCT"]["uiModel"]) == 30 )
     and ( uiFGetIDWithChannelNameWith(sIndex) >  16 ) ):
    self.ui.groupBox_AUX_PT2.setTitle("Point 2")
   else:
    self.ui.groupBox_AUX_PT2.setTitle(tCalibChannelInfo[sIndex]["Point"][1]["sName"])
   self.ui.groupBox_AUX_PT2.setVisible(True)
   self.siCalibrationPointSelectConnectSignal.emit( sConfigName, ucChannel, tChannel, self.ui.widget_AuxPt2Value, 1, ttConfig )
  else:
   self.ui.groupBox_AUX_PT2.setVisible(False)
  #-- Point 3 --
  if( tCalibChannelInfo[sIndex]["ucPoint"] > 2 ):
   if(   ( sIndex == "PH" )
     and (  ( ttConfig["PRODUCT"]["uiModel"] == 17 )
         or ( ttConfig["PRODUCT"]["uiModel"] ==  8 )
         or ( ttConfig["PRODUCT"]["uiModel"] == 27 )
         or ( ttConfig["PRODUCT"]["uiModel"] == 28 ) ) ):
    ...
    self.siCalibrationPointSelectConnectSignal.emit( sConfigName, ucChannel, tChannel, self.ui.widget_AuxPt3Value, 2, ttConfig )
  else:
   self.ui.groupBox_AUX_PT3.setVisible(False)
  ...
  #---------------
  # Si pH : AP-700/AP-800 ancien
  if( sIndex == "PH" ):
   if(  ( ttConfig["PRODUCT"]["uiModel"] == 17 )
     or ( ttConfig["PRODUCT"]["uiModel"] ==  8 )
     or ( ttConfig["PRODUCT"]["uiModel"] == 27 )
     or ( ttConfig["PRODUCT"]["uiModel"] == 28 ) ):
    print('sIndex == "PH"')
  ...
  if( sIndex == "EC" ):
   ...
   # Champs EC Cal value comboboxValue
   # Selon la version
   # EC_STD_BOX
   if( ttConfig["PRODUCT"]["ProBE_SW"] < 3 ):
    self.ui.comboBox_ECCalValue_Point.clear()
    self.ui.comboBox_ECCalValue_Point.addItems(["RapidCal",
                                                "1413 uS",
                                                "12,880 uS",
                                                "SC-35"])
   # EC_STD_BOX_V5
  ...
  if( uiFGetIDWithChannelNameWith(sIndex) > 16 ):
   ...
   #-- AP-Lite
   if( int(ttConfig["PRODUCT"]["uiModel"])   == 30 ):
    # Cache calibration parameter GS Factor
    self.ui.groupBox_CalibrationParameters.setVisible(False)
   # Pas AP-Lite
   else:
    # Affiche
  ...
  # Si pas AP-Lite
  if(   ( "uiCalReport" in tChannel["Point"][0] )
    and ( tChannel["Point"][0]["uiCalReport"] != None ) ):
   # Pas de valeur de calibration
   ...
   ##self.ui.label_AuxPt1Value.setText("point 1")
   self.ui.label_AuxPt1Value.setText(tCalibChannelInfo[sIndex]["Point"][0]["sName"])
  ...
  if( tChannel["ucPoint"] > 1 ):
   ...
   # Si pas AP-Lite
   if(   ( "uiCalReport" in tChannel["Point"][1] )
     and ( tChannel["Point"][1]["uiCalReport"] != None ) ):
     # Pas de valeur de calibration
     self.ui.label_AuxPt2Value.setText( str(tChannel["Point"][1]["uiCalReport"])+" "+tChannel["Point"][1]["sCalReportUnit"] )
   else:
    ##self.ui.label_txtSN_10.setText("Click to calibrate")
    ##self.ui.label_AuxPt2Value.setText(tCalibChannelInfo[sIndex]["Point"][1]["sName"])
    # Si capteur optique
    if( uiFGetIDWithChannelNameWith(sIndex) > 16 ):
     self.ui.label_AuxPt2Value.setText("point 2")
    # Si Tbd
    else:
     self.ui.label_AuxPt2Value.setText(tCalibChannelInfo[sIndex]["Point"][1]["sName"])
  ...
  if( tChannel["ucPoint"] > 2 ):
   ...
   # Si pas AP-Lite
   if(   ( "uiCalReport" in tChannel["Point"][2] )
     and ( tChannel["Point"][2]["uiCalReport"] != None ) ):
    # Pas de valeur de calibration
    self.ui.label_AuxPt3Value.setText( str(tChannel["Point"][2]["uiCalReport"])+" "+tChannel["Point"][2]["sCalReportUnit"] )
   else:
    self.ui.label_AuxPt3Value.setText(tCalibChannelInfo[sIndex]["Point"][2]["sName"])
  # Descriptif
 ...
 #----------------------------------------------
 def vFCALIBRATIONPointNavSelectionDynamic( self, sConfigName, ucChannel, tChannel, tWidget, uiPoint, ttConfig ):
  # Changement d'onglet
  ...
  # Si définie
  if( "uiCalReport" in tChannel["Point"][uiPoint] ):
   # Last calibration value
   self.ui.widget_32.setVisible(True)
  ...
  #-- Si AP-Lite et point 2
  if(   ( int(ttConfig["PRODUCT"]["uiModel"]) == 30 )
    and ( uiPoint == 1 )
    and ( ttConfig["SENSOR"]["AUX1"]["uiIndex"] > 16 ) ):
   self.ui.groupBox_CalibrationParameters_Point.setVisible(True)
   self.ui.widget_ProbeAUXCalValue_Point.setVisible(True)
  else:
 ...
 def vFCALIBRATION_ECCalValueChange( self ):
  ...
  # RapidCal
   ...
   self.ui.spinBox_ECCalValue_2_Point_2.setValue(2570)
  # Si en mode 1413 uS
  elif( self.ui.comboBox_ECCalValue_Point.currentText() == "1413 uS"  ):
   self.ui.widget_ECCalValue_Point_2.setVisible(False)
   self.ui.widget_ECCalValue_Point_3.setVisible(True)
   self.ui.spinBox_ECCalValue_2_Point_2.setValue(1413)
  # Si en mode 12,880 uS
  elif( self.ui.comboBox_ECCalValue_Point.currentText() == "12,880 uS"  ):
   self.ui.widget_ECCalValue_Point_2.setVisible(False)
   self.ui.widget_ECCalValue_Point_3.setVisible(False)
  # SC-35
  else:
